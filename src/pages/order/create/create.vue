<template>
	<view>
		...
		<view class="order" v-if="orderDetail.products">
			...
			<!--预售-->
			<view v-if="params.type == 'presell_buy'" class="dz-list">
				<dz-cell-item
					center
				...
				>
					<template #right-icon>
						<dz-radio-group
							...
							:disabled="orderDetail.preview && parseInt(orderDetail.preview.final_payment_money) == 0"
							:active-color="theme.dzBaseColor"
						>
							<dz-radio :name="0" :label="language.orderCreate.presellOrderPay" customStyle="marginRight: 20rpx"></dz-radio>
							<dz-radio :name="1" :label="language.orderCreate.presellOrderFullPay"></dz-radio>
						</dz-radio-group>
					</template>
				</dz-cell-item>
			</view>
			...
			<view class="dz-list ios-bottom">
				...
				<dz-cell-item
					...
					:value="api.formatString(language.orderCreate.price, api.toFloat(discountAmountView, 2).toFixed(2))"
					:arrow="
						(parseFloat(discountAmountView) > 0 && orderDetail.marketing_full_details.length) ||
						(couponTabs.length > 0 && isShowConpon && couponItem.money)
							? true
							: false
					"
					:border-bottom="false"
				...
				></dz-cell-item>
				<dz-cell-item
					v-if="params.type == 'presell_buy' && isFullPayment == 0"
					center
					title="备货后付余款"
					:value="api.formatString(language.orderCreate.price, api.toFloat(discountAmountView, 2).toFixed(2))"
					:arrow="
			...
			<view class="footer">
				<view class="footer-bottom">
						...
						<text class="price text-bold" :style="{ color: theme.dzBaseColor }">
							{{
								`${realAmount > 0 ? realAmount : ''} ${
									(maxUsePoint > 0 &&
										(isUsePoint
											? ` ${realAmount > 0 ? '+' : ''} ${maxUsePoint}${api.formatString(
													language.orderCreate.point,
													shopSetting.point_front_name
											  )}`
											: '')) ||
									(orderDetail.preview && orderDetail.preview.point
										? ` ${realAmount > 0 ? '+' : ''} ${orderDetail.preview && orderDetail.preview.point}${api.formatString(
												language.orderCreate.point,
												shopSetting.point_front_name
										  )}`
										: '')
					...
					<view v-if="orderDetail.preview && userInfo.account && userInfo.account.user_integral >= orderDetail.preview.point">
						<dz-button
							:custom-style="{
								width: '260rpx',
								height: '80rpx',
		...
		<dz-popup v-model:show="isShow" mode="bottom" border-radius="40" closeIcon="roundclose" closeIconSize="40" :closeable="true">
			<view class="dz-coupon__box">
				...
				<view v-if="couponCurrent != 1" class="dz-m-20">
					<dz-button
						:custom-style="{
							width: '100%',
		...
		<dz-popup v-model:show="discountAmountShow" mode="bottom" :border-radius="40" closeIcon="roundclose" :closeIconSize="40" :closeable="true">
			<view class="dz-discount-amount">
				...
				<scroll-view scroll-y style="min-height: 600rpx; color: #666">
					...
					<view v-if="couponTabs.length > 0 && isShowConpon && couponItem.money" class="dz-flex dz-row-between">
...
onLoad(async (options) => {
	...
	const conponType = ['join_buy', 'combination', 'limited_buy'];
	if (conponType.includes(data.params.type) || (data.params.point_exchange_type && data.params.point_exchange_type == 4)) {
...
// 是否使用积分抵扣
function handleIsUsePoint(value: boolean) {
	if (value) {
...
async function getOrderDetail() {
	await uni.$api.http
		...
		.then((res) => {
			...
			//优惠券
			if ('combination,join_buy,limited_buy'.indexOf(data.params.type) !== -1) {
				data.couponItem = {};
			} else {
				let couponFee = [];
				let couponArr = [];
				...
				if (couponFee.length > 0) {
					...
					data.couponItem.money = couponViewMoney.value;
...
async function submit() {
	//锁粉方式为首次下单或首次付款，且用户没有推荐人，且存在推荐码，需要提交最后的推荐人信息
	if ([2, 3].includes(parseInt(shopSetting.value.member_parent_type)) && userInfo.value.pid == 0 && promoCode.value) {
		uni.$api.http.post(uni.$api.apiMember.setParent, {
...
function selectCoupon(item: any) {
...
function couponSubmit() {
	...
	if (data.activeCoupon && data.activeCoupon.id) {
		data.couponItem.money = couponViewMoney.value;
	}
	isShow.value = false;
}
...
const { orderDetail, addressData, currentShippingType, params, currentPickupPoint, pointConfig, couponItem, invoiceItem, coupon, activeCoupon } = {
	...toRefs(data),
};
</script>
