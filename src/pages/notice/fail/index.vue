<template>
	<view>
		<dz-empty name="roundclosefill" iconSize="180" :iconColor="theme.dzTypeWarning" text="" :iconStyle="{ marginTop: '200rpx' }">
			<template #bottom>
...
const tilte = ref<string>(language.notice.failTitle);
const sub = ref<string>(language.notice.failSub);

onLoad(async (options) => {
	await uni.$onLaunched;
	uni.setNavigationBarTitle({ title: uni.$api.language.notice.failTitle });
	if (options.title) {
...
function getMemberInfo() {
	uni.$api.http.get(uni.$api.apiShop.memberInfo).then((res) => {
