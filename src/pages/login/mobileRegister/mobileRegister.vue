...
const lastMobileRegisterCaptchaTimeStamp = computed(() => {
	return userStore.getLastMobileRegisterCaptchaTimeStamp;
});
...
function computeLastCaptcha() {
	...
	let timeDiff = timeNow - lastMobileRegisterCaptchaTimeStamp.value;
...
function getCode() {
	...
	uni.$api.http
		...
		.then((res) => {
			...
			userStore.setLastCaptchaTimeStamp('lastMobileRegisterCaptchaTimeStamp', parseInt(new Date().getTime() / 1000));
...
function submit() {
	unref(formRef)
		...
		.then((res) => {
			...
			uni.$api.http
				.post(uni.$api.apiMember.mobileRegister, {
					...formData,
				})
				.then((res) => {
