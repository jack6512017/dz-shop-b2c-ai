		...
		<view class="container">
			...
			<view class="form-wrap">
				<view class="dz-m-t-60">
					...
					>
						<template #prefix>
							<view class="dz-m-r-15 dz-main-color" style="width: 146rpx" @tap="handleArea">
								{{ areaName }}
				...
				<view class="dz-m-t-30">
					...
					>
						<template #prefix>
							<view class="dz-m-r-15 dz-main-color" style="width: 146rpx">密码</view>
						</template>
						<template #suffix>
				...
				<view class="dz-text-right dz-p-20 dz-font-24 dz-tips-color" @tap="handelMobileRegister()">立即注册</view>
				<view class="dz-m-t-60">
					<dz-button
						v-if="!closeLogin"
					...
					<dz-button
						...
						:custom-style="{
							width: '100%',
							height: '95rpx',
							background: submitDisabled || isLoaidng ? theme.dzBaseDisabled : theme.dzBaseColor,
				...
				<view class="dz-m-t-20 dz-font-24 dz-text-center dz-tips-color">{{ language.login.passwordLoginTips }}</view>
			</view>
			<view
				v-if="isMobileLogin || (showAppleLogin && appAppleLogin) || showWechatLogin || usernamePasswordLogin"
				class="dz-text-center dz-m-t-80 dz-tips-color dz-font-26"
				>其它登录方式</view
			>
...
const { lastInputMobile, lastSelectMobileArea, promoCode, dealerCode, storeId, thirdPartyUserInfo } = userStore.getData;
...
const submitDisabled = computed(() => {
	if (uni.$api.isMobile(areaCode.value + mobile.value, areaCode.value) && password.value) {
		return false;
	}
	return true;
});
...
function handelMobileRegister() {
	uni.$api.router.push({
		route: uni.$api.routesConfig.usernameMobileRegister,
	});
}
...
async function handleSubmit() {
	...
	if (!uni.$api.isMobile(String(areaCode.value) + mobile.value, areaCode.value)) {
		isLoaidng.value = false;
		uni.$api.toast(language.login.mobileError);
		return;
	}
	if (!password.value) {
		isLoaidng.value = false;
		uni.$api.toast(language.login.passwordError);
		return;
	}
	let formData = {
		mobile: mobile.value,
	...
	await uni.$api.http
		.post(uni.$api.apiMember.mobilePasswordLogin, {
			...formData,
		})
		.then((res) => {
