...
import popupAdv from '@/pages/main/index/components/popupAdv/popupAdv.vue';
import wechatGuide from '@/pages/main/index/components/wechatGuide/wechatGuide.vue';
...
async function getCustomPage() {
	...
	const query = {};
	if (data.pageType) {
		query.type = data.pageType;
	}
	await uni.$api.http
		.get(uni.$api.apiStore.homeIndex, {
			params: {
...
function sharePath() {
	...
	// #ifdef MP
	url = uni.$api.routesConfig.storeIndex.path;
	// #endif
	// #ifndef MP
	url = `${domain}${uni.$api.routesConfig.storeIndex.path}`;
	// #endif
	if (userStore.hasLogin) {
