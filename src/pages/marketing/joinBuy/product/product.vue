...
async function getProductDetail() {
	await uni.$api.http
		...
		.then((res) => {
			...
			//初始化微信客服
			// #ifdef MP-WEIXIN
			wxCustomerServicePath.value = `${uni.$api.routesConfig.joinBuyProduct.path}.html?id=${productId.value}`;
			// #endif
		})
		.catch((err) => {
			pageLoadingShow.value = true;
...
function updateShare() {
	...
	if (uni.$api.h5IsWechat() && shopSetting.value.wechat_appid != '') {
		uni.$api.wxHelper.wxShare(
			...
			uni.$api.language.joinBuy.joinBuyShare,
			data.productDetail.name,
			sharePath(),
	...
	// #endif
	//初始化小程序分享
	data.mpShare.title = uni.$api.language.joinBuy.joinBuyShare;
	data.mpShare.path = sharePath();
	data.mpShare.imageUrl = data.productDetail.covers[0];
	//初始化APP分享
	data.shareData = {
		shareUrl: sharePath(),
		shareTitle: uni.$api.language.joinBuy.joinBuyShare,
...
function sharePath() {
	...
	// #ifdef MP
	url = `${uni.$api.routesConfig.joinBuyProduct.path}?id=${productId.value}`;
	// #endif
	// #ifndef MP
	url = `${domain}${uni.$api.routesConfig.joinBuyProduct.path}?id=${productId.value}`;
	// #endif
	if (userStore.hasLogin) {
