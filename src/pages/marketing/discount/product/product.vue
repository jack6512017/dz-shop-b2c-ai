...
onLoad(async (options) => {
	...
	marketingId.value = options.marketing_id;
...
async function getProductDetail() {
	await uni.$api.http
		...
		.then((res) => {
			...
			//初始化微信客服
			// #ifdef MP-WEIXIN
			wxCustomerServicePath.value = `${uni.$api.routesConfig.discountProduct.path}.html?id=${productId.value}`;
			// #endif
		})
		.catch((err) => {
			pageLoadingShow.value = true;
...
function updateShare() {
	...
	if (uni.$api.h5IsWechat() && shopSetting.value.wechat_appid != '') {
		uni.$api.wxHelper.wxShare(
			...
			shopSetting.value.discount_share_title,
			data.productDetail.name,
			sharePath(),
	...
	// #endif
	//初始化小程序分享
	data.mpShare.title = shopSetting.value.discount_share_title;
	data.mpShare.path = sharePath();
	data.mpShare.imageUrl = data.productDetail.covers[0];
	//初始化APP分享
	data.shareData = {
		shareUrl: sharePath(),
		shareTitle: shopSetting.value.discount_share_title,
...
function sharePath() {
	...
	// #ifdef MP
	url = `${uni.$api.routesConfig.discountProduct.path}?id=${productId.value}`;
	// #endif
	// #ifndef MP
	url = `${domain}${uni.$api.routesConfig.discountProduct.path}?id=${productId.value}`;
	// #endif
	if (userStore.hasLogin) {
