		...
		<view :id="id">
			<scroll-view scroll-x class="dz-scroll-view" :scroll-left="scrollLeft" scroll-with-animation>
				<view class="dz-scroll-box" :class="{ 'dz-tabs-scorll-flex': !isScroll }" :style="{ height: height + 'rpx' }">
					<view
						class="dz-tab-item dz-line-1"
						:id="'dz-tab-item-' + index"
...
const tabQueryInfo = ref([]); // 存放对tab菜单查询后的节点信息
const componentWidth = ref<number>(0); // 屏幕宽度，单位为px
const parentLeft = ref<number>(0); // 父元素(tabs组件)到屏幕左边的距离
const id = uni.$api.guid();
const currentIndex = ref<number | string>(props.current);
...
const tabItemStyle = computed(() => (index) => {
	let style = {
		height: props.height + 'rpx',
		'line-height': props.height + 'rpx',
		'font-size': props.fontSize + 'rpx',
		padding: props.isScroll ? `0 ${props.gutter}rpx` : '',
		borderRadius: props.buttonRadius + 'rpx',
		flex: props.isScroll ? 'auto' : '1',
		width: uni.$api.addUnit(props.itemWidth),
		background: props.inactiveBgColor,
	};
	// 字体加粗
...
async function init() {
	// 获取tabs组件的尺寸信息
	let tabRect = await uni.$api.getRect('#' + id);
	// tabs组件距离屏幕左边的宽度
...
function scrollByIndex() {
	...
	let _scrollLeft = offsetLeft - (componentWidth.value - tabWidth) / 2;
	scrollLeft.value = _scrollLeft < 0 ? 0 : _scrollLeft;
}
