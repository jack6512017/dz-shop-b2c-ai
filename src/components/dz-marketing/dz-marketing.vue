	...
	>
		<dz-cell-item
			...
			:title-style="{
				...
				color: propsConfig.titleColor,
			}"
			:value-style="{
				fontSize: `${propsConfig.titleRightSize * 2}rpx`,
				color: propsConfig.titleRightColor,
			}"
		...
		>
			<view class="h-item" v-for="(item, index) in list" :key="index" @tap.stop="productTab(item)">
				<dz-image :src="item.picture" :width="192" :height="192" :borderRadius="15"></dz-image>
						...
						<view class="dz-pro-content" v-if="item" :class="[propsConfig.styleGroup == 'oneLine' ? 'dz-m-l-20' : '']">
							...
							<view :style="{ color: theme.dzBaseColor }">
								<dz-tag v-if="propsConfig.tagShow == 1" size="mini" bgColor="#fff" :borderColor="theme.dzBaseColor" :color="theme.dzBaseColor">
									<text v-if="marketingType == 'dz-haggle'" class="dz-font-10">低价</text>
									<text v-if="marketingType == 'dz-limited'" class="dz-font-10"> 限量价</text>
									<text v-if="marketingType == 'dz-discount'" class="dz-font-10">折扣价</text>
									<text v-if="marketingType == 'dz-join-buy'" class="dz-font-10">拼团价</text>
									<text v-if="marketingType == 'dz-presale'" class="dz-font-10">定金</text>
								</dz-tag>
								<view class="dz-flex dz-row-between">
									<view class="dz-pro-price">
										<text class="dz-sale-price dz-font-price" :style="{ color: theme.dzBaseColor }"> {{ getPrice(item) }}</text>
										<text
											v-if="propsConfig.marketPriceShow && item.market_price"
											class="dz-m-l-10 dz-factory-price dz-font-price dz-tips-color"
											style="text-decoration: line-through"
										>
											{{
												marketingType == 'dz-discount' ? api.toFloat(item.price).toFixed(2) : api.toFloat(item.market_price).toFixed(2)
											}}
										</text>
									</view>
									<view v-if="propsConfig.buttonShow == 1">
										...
										>
											<block v-if="marketingType == 'dz-haggle'">去砍价</block>
											<block v-if="marketingType == 'dz-limited'"> 去抢购</block>
											<block v-if="marketingType == 'dz-discount'">去抢购</block>
											<block v-if="marketingType == 'dz-join-buy'">去拼团</block>
											<block v-if="marketingType == 'dz-presale'">付定金</block>
										</dz-button>
									</view>
								</view>
							</view>
							...
							<view>
								...
								<view class="dz-flex dz-row-between">
									<view class="dz-pro-price">
										...
										<text
											v-if="propsConfig.marketPriceShow && item.market_price"
											class="dz-m-l-10 dz-factory-price dz-font-price dz-tips-color dz-font-28"
											style="text-decoration: line-through"
										>
											{{
												marketingType == 'dz-discount' ? api.toFloat(item.price).toFixed(2) : api.toFloat(item.market_price).toFixed(2)
				...
				<view class="dz-p-10">
					...
					<view class="dz-flex dz-row-between dz-col-bottom dz-m-t-10">
						<view class="dz-flex dz-col-bottom">
							...
							<text
								v-if="propsConfig.marketPriceShow && item.market_price"
								class="dz-m-l-6 dz-font-28 dz-tips-color"
								style="text-decoration: line-through"
							>
								￥
								{{ marketingType == 'dz-discount' ? api.toFloat(item.price).toFixed(2) : api.toFloat(item.market_price).toFixed(2) }}
		...
		<view v-if="propsConfig.styleGroup == 'threeLine'" class="product-threeLine" :class="[propsConfig.titleShow == 1 ? '' : 'dz-m-t-10']">
			<dz-row>
				<dz-col v-for="(item, index) in list" :key="index" :gutter="12" :span="4">
					<view class="item dz-m-t-10" @tap.stop="productTab(item)">
						<dz-image :src="item.picture" width="100%" :height="(750 - propsConfig.marginLeftRight * 4 - 64) / 3" borderRadius="15"></dz-image>
						<view
							v-if="propsConfig.productNameShow == 1"
							class="name dz-line-2 dz-m-t-5"
							:style="{
								width: `${(750 - propsConfig.marginLeftRight * 4 - 64) / 3}rpx`,
							}"
						>
							{{ item.name }}
						</view>
						<view class="dz-text-center dz-line-1 dz-m-t-5">
							...
							<text class="dz-font-28 dz-font-weight" :style="{ color: theme.dzBaseColor }">{{ getPrice(item) }}</text>
							<text
								v-if="propsConfig.marketPriceShow && item.market_price"
								class="dz-tips-color"
...
function recommendTap() {
	if (!props.marketingType) return;
	let path;
...
function marketingPrice(val: object) {
	//限时折扣
...
function getPrice(item: obejct): number {
	if (!props.marketingType) return;
	let price = item.price;
	switch (props.marketingType) {
		case 'dz-haggle':
			price = uni.$api.toFixed(item.price - (1 - uni.$api.toFloat(item.haggle.min_rate / 100)) * item.price, 2);
			break;
		case 'dz-discount':
			price = marketingPrice(item);
			break;
		case 'dz-join-buy':
			price = item.join_buy_price;
			break;
		case 'dz-limited':
			price = item.limited_buy.min_price;
			break;
	}
	return parseFloat(price || '0').toFixed(2);
}

function productTab(item: object) {
	if (!props.marketingType) return;
	let url;
	const params = {
		id: item.id,
	};
	if (props.marketingType == 'dz-discount') {
		params.marketing_id = item.discount.discount_id;
	}
	switch (props.marketingType) {
		case 'dz-haggle':
			url = 'haggleProduct';
