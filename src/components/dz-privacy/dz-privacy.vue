<template>
	<view class="dz-privacy" :class="[{ 'dz-bottom': position == 'bottom' }]" v-if="showPrivacy">
		<view class="dz-privacy-container" :style="{ '--color': color, '--bgcolor': bgcolor, '--contenth': contentHeight }">
			<view class="title">
				{{ title }}
			</view>
			<view class="content"
				>{{ predesc }}<text @click.stop="handleOpenPrivacyContract">{{ privacyContractNameCustom || privacyContractName }}</text
				>。{{ subdesc }}
			</view>
			<view class="footer">
